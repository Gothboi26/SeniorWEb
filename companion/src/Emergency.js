...
const Emergency = ({ role, handleLogout }) => {
  ...
  const handleEmergencyClick = async (type) => {
    ...
    try {
      const res = await fetch("https://backend-production-4629.up.railway.app/php/submit_emergency.php", {
        method: "POST",
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const res = await fetch("https://backend-production-4629.up.railway.app/php/get_latest_emergency_status.php", {
          method: "GET",
          credentials: "include",
        });
        const data = await res.json();
